...
import { convertTime, reConverPhone, useLogLocation } from '../../service';
...
function LogLocation() {
  ...
  const handleSearch = () => {
    if (startDate && endDate) {
      ...
      getLogLocation.run(queryParams);
    } else {
  ...
  ////call api////
  const getLogLocation = useLogLocation({
    onSuccess: (resp) => {
      if (resp.data) {
        setR(resp?.data);
      }
  ...
  const cols = [
    ...
    },
    {
      ...
      render: (_: number, record: any) => (
        <>{record.in_safezone ? record.name_safezone : <div>Ngoài vùng khai báo</div>}</>
      ),
    },
    {
      title: 'Khoảng cách so với vùng an toàn gần nhất(m)',
      dataIndex: 'distance_with_safezone',
      key: 'distance_with_safezone',
    },
    {
      title: 'Cập nhật lúc',
  ...
  return (
    <div>
      <ActionHead
        title={`Lịch sử vị trí - Trưởng nhóm #${
      ...
      <div>
        <CustomTable
          ...
          loading={getLogLocation.loading}
        />
      </div>
    </div>
  );
