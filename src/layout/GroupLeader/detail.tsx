...
import { converPhone, convertTime, formatDate, useDetailUser } from './service';
function UserDetail() {
  ...
  const detailTVTB = useDetailUser({
    onSuccess: (resp) => {
      setR(resp.data);
    },
    onError: (e) => {
  ...
  return (
    <div>
      <ActionHead title='Thông tin thành viên được kết nối thiết bị'>
        <Button text='Trở lại' handleSubmit={handleGoBack} />
      </ActionHead>
      ...
      <div className={styles.two_columns_container}>
        ...
        <div className={styles.column2}>
          ...
          <div className={styles.row}>
            <div className={styles.text_row}>{converPhone(r?.phone_request_connect)}</div>
          </div>
          <div className={styles.row}>
